...
function tempera_singlecolumn_output($data){
	extract($data);
	?>
		<div class="pp-column column<?php echo intval($colno); ?>">
			<?php if (!empty($image)) {	?>
				<a href="<?php echo esc_url($link); ?>" <?php echo esc_html($blank); ?> class="clickable-column">
					<?php if (!empty($title)) { ?><h3 class='column-header-image'><?php echo esc_html($title) ?></h3><?php } ?>
				</a>

				<div class="column-image">
					<div class="column-image-inside">  </div>
						<a href="<?php echo esc_url($link); ?>" <?php echo esc_html($blank); ?> class="clickable-column">
							<img src="<?php echo esc_url($image) ?>" alt="<?php echo (!empty($title)?esc_attr($title):''); ?>" />
						</a>
					...
					<?php if ( !empty($readmore) && !empty($link) ) { ?>
						<div class="columnmore">
							<a href="<?php echo esc_url($link); ?>" <?php echo esc_html($blank); ?>><?php echo esc_html($readmore) ?> <i class="column-arrow"></i> </a>
						</div>
					<?php } ?>
				</div><!--column-image-->
			<?php } ?>
