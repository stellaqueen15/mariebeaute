	...
	function catchresponsive_custom_breadcrumbs( $showOnHome, $delimiter ) {
		...
		if ( is_front_page() ) {
			...
			elseif ( is_category() ) {
				...
				echo $before . sprintf( $text['category'], '<span class="archive-text">', '&nbsp &raquo &nbsp</span>' . single_cat_title( '', false ) ) . $after;
			...
			elseif ( is_tag() ) {
				echo $before . sprintf( $text['tag'] , '<span class="tag-text">', '&nbsp &raquo &nbsp</span>' . single_tag_title( '', false ) ) . $after;
