 ...
 * Adopted from Dimox
 *
 * @since Catch_Responsive
 */
if( !class_exists('Catchresponsive_Json_Ld_Schema')){
class Catchresponsive_Json_Ld_Schema {
	private $position        = 0;
	...
	function catchresponsive_custom_breadcrumbs_json_ld( $show_on_home ) {
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'catch-responsive' ); // text for the 'Home' link
		$text['category'] = __( 'Archive for ', 'catch-responsive' ); // text for a category page
		$text['search']   = __( 'Search results for: ', 'catch-responsive' ); // text for a search results page
		$text['tag']      = __( 'Posts tagged ', 'catch-responsive' ); // text for a tag page
		$text['author']   = __( 'View all posts by ', 'catch-responsive' ); // text for an author page
	...
	function json_ld_breadcrumbs() {
		...
		if ( isset( $options['breadcumb_option'] ) && $options['breadcumb_option'] ) {
			...
			$breadcrumb = $this->catchresponsive_custom_breadcrumbs_json_ld( $show_on_home );
			$json       = '';
...
new Catchresponsive_Json_Ld_Schema();
}
